...
- ✅ ~~(Must, S9) Le state est muté directement~~ — corrigé le 20/09/2026 dans **les deux** formulaires. `[...albums]` ne copiait que le tableau, pas les objets : `newAlbums[i][cat] = value` modifiait donc l'objet stocké dans le state `bands`, et la modification survivait à « Cancel ». Remplacé par `setAlbums(albums.map((a, i) => i === index ? { ...a, [cat]: value } : a))`. **Protégé par un test de régression** (`UpdateBandForm.test.jsx` → « does not modify the band in the bands state while typing »), vérifié comme échouant sur l'ancien code. 🎤 **À savoir expliquer** : en React on ne mute jamais le state, on le remplace — React compare les références pour décider de re-render. En Angular, muter fonctionne (Zone.js détecte le changement) : c'est LA différence de fond entre les deux.
- ✅ ~~(Must, S9) Plantage si un groupe local n'a pas de tableau `albums`/`members`~~ — corrigé le 20/09/2026 (`band.albums || []`).
- ✅ ~~(Should, S9) Les genres ne sont pas nettoyés~~ — corrigé le 20/09/2026 (`.map(genre => genre.trim())` dans les deux formulaires).
- ✅ ~~(Must, S9) Les formulaires de recherche et de connexion rechargent toute la page~~ — corrigé le 20/09/2026 (`onSubmit` + `preventDefault()`).
- (Should, S9) **`key={index}` dans les formulaires** (`AddBandForm.jsx:233,285` et `UpdateBandForm.jsx:270,328`) : les lignes d'album/membre sont ajoutables et supprimables, donc l'index est instable (supprimer la 1ʳᵉ ligne décale tout). Corriger demande d'ajouter un id local à chaque ligne → à faire **en même temps** que la correction de l'immutabilité ci-dessus. Les listes en lecture seule de `BandsId.jsx` sont déjà corrigées.
...
- ✅ ~~(Should, S9) CSS dupliqué entre `AddBandForm.css` et `UpdateBand.css`~~ — corrigé le 20/09/2026 : fusionnés dans `BandForm.css` (400 → 200 lignes), classes partagées (`.band-form-container`, `.band-form`, `.submit-btn`, `.addmore-btn`), les deux anciens fichiers supprimés.
- (Should, S9) **JSX dupliqué entre `AddBandForm.jsx` et `UpdateBandForm.jsx`** : 286 lignes identiques sur 343 / 398. Les deux ont exactement 13 `.input-group`, 5 `.input-row`, 12 `<input>`, 3 `<select>` et les mêmes 3 sections. Piste : extraire un composant `FormField` (label + input), puis éventuellement un `BandForm` partagé. À faire en même temps que le passage aux Actions React 19.
...
- ✅ ~~(Could, S16) `src/index.css` entièrement commenté mais importé + `import React` inutiles~~ — corrigé le 20/09/2026 (fichier supprimé, `import React` retiré de 13 fichiers).
- ✅ ~~(Must, S6 ou S9) json-server : l'id du client écrasé par le serveur~~ — corrigé le 20/09/2026 : `AddBandForm.jsx` lit maintenant `response.data`. **Conséquence : json-server peut enfin être mis à jour**, et `uuid` retiré (l'id est encore envoyé mais n'est plus utilisé côté state).
- ✅ ~~json-server : version non épinglée côté back, pas de validation, pas de proxy possible~~ — **remplacé par un backend Express** le 20/09/2026 (`server.js`, ~140 lignes : CRUD + validation + proxy MusicBrainz). json-server retiré des deux `package.json`.
- ✅ ~~MusicBrainz sans `User-Agent`~~ — résolu le 20/09/2026 par le proxy `/api/mb/*`. **Décision : aucune donnée personnelle dans le code.** Le défaut est neutre (`EncyclopediaPunkorum/1.0 (student project)`) ; un contact peut être ajouté via la variable d'environnement `MB_USER_AGENT` chez l'hébergeur, sans jamais toucher au dépôt.
- ✅ ~~`db.json` en double~~ — supprimé du front le 20/09/2026. Seul `encyclopedia-punkorum-ironhack-backend/db.json` fait foi.
- 🔵 **(Must, S13) Rendre le stockage persistant** — **décidé le 20/09/2026** : on assume pour l'instant, et on le corrige avant le déploiement final.
  - **Le problème** : `db.json` est un fichier. Sur l'offre gratuite de Render, le système de fichiers est éphémère → tout groupe ajouté en ligne disparaît au redémarrage. ⚠️ SQLite ne résout rien (c'est aussi un fichier).
  - **La solution prévue** : une base Postgres hébergée (Render en propose une gratuite). Compter 1 à 2 week-ends : créer la base, réécrire les 5 routes de `server.js`, migrer les 16 groupes. ⚠️ La base gratuite de Render **expire au bout de 30 jours** — donc la créer *tard*, pas en S13, et vérifier qu'elle vit encore début février.
  - **En attendant, la réponse pour le jury** : *"I know the limit: my data lives in a JSON file, so it does not survive a restart on a free host. The fix is a hosted database — that is my next step."* Connaître la limite de son archi vaut mieux que de prétendre qu'il n'y en a pas.
- ✅ ~~(Should, S13) Écritures concurrentes non sûres~~ — corrigé le 20/09/2026. **Trouvé par un test, pas à l'œil** : 5 créations simultanées ne donnaient que 2 groupes. Cause : sérialiser seulement l'écriture ne suffit pas, les requêtes lisaient toutes le même fichier périmé avant d'entrer dans la file. C'est tout le cycle **lecture → modification → écriture** qui passe désormais dans une file (`updateBands`). Écriture atomique en prime (fichier `.tmp` puis `rename`). 🎤 *"A database would do this for me with a transaction — with a JSON file I had to serialise it myself."*
- ✅ ~~(Should) Le rate limit MusicBrainz~~ — atténué le 20/09/2026 : le proxy **cache** les réponses 5 minutes, et sert une réponse **périmée plutôt que rien** si MusicBrainz répond 503. En-tête `X-Cache: HIT/MISS/STALE` pour le vérifier.
- (Should) **Le rate limit MusicBrainz est réel** — constaté le 20/09/2026 pendant les tests : plusieurs appels rapprochés → **HTTP 503**. L'app dégrade proprement (les groupes locaux s'affichent quand même, cf. le `try/catch` dans `getAllBands`), mais la liste MusicBrainz disparaît sans explication. Pistes : mettre un message « MusicBrainz momentanément indisponible », et/ou un cache côté serveur dans le proxy (les résultats changent rarement).
- (Could) **`setTimeout(…, 1000)` dans `BandsId.jsx`** : contournement du rate limit MusicBrainz qui ralentit chaque page détail d'une seconde. Maintenant que le proxy existe, la temporisation aurait plutôt sa place côté serveur.
- ✅ ~~(Should, S5) `Routers.tsx` type tout en `any`~~ — **le front est passé en TypeScript strict** le 20/09/2026. 32 fichiers migrés, `tsconfig.json` en `strict`, `noImplicitAny`, `noUnusedLocals`. **0 `any` dans le projet**, garanti par la règle ESLint `@typescript-eslint/no-explicit-any`. Nouveau script `npm run typecheck`, et `npm run build` vérifie les types avant de compiler.
  - **Le cœur : `src/types.ts`.** `Band` est une **union discriminée** par `source`. Le compilateur refuse `band.albums` tant que `band.source === 'local'` n'a pas été testé → le piège « deux formats » est devenu une erreur de compilation.
  - **Type guards** dans `BandsId.tsx` (`isLocalAlbum`, `isLocalMember`) : pour une liste mélangée, il faut narrower sur l'**élément**, pas sur `band.source` — TypeScript ne peut pas suivre la seconde forme.
  - **Ce que le compilateur a trouvé tout seul** : 2 props mortes (`bands` passée à `AddBandForm`, `setBands` passée à `BandsId` — aucun des deux ne les utilisait), et `member.attributes?.length > 0` qui comparait `undefined > 0`.
  - 🎤 *"TypeScript turned my main design risk — two incompatible band shapes — into a compiler error. It also found two props I was passing to components that never used them."*
